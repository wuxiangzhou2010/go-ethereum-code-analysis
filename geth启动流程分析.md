...
app是一个[三方包](gopkg.in/urfave/cli.v1)的实例。 这个三方包的用法大致就是首先构造这个app对象。 通过代码配置app对象的行为，提供一些回调函数。然后运行的时候直接在main函数里面运行 `app.Run(os.Args)`就行了。
...
makeConfigNode。 这个函数主要是通过`配置文件`和`flag`来`生成整个系统的运行配置`。
...
整个启动过程其实就是`解析参数`。然后`创建和启动节点`。 然后把服务注入到节点中。 所有跟以太坊相关的功能都是以服务的形式实现的。
