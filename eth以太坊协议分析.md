...
// 根据配置中的同步模式启动全节点或者轻节点
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
    var err error
    if cfg.SyncMode == downloader.LightSync {
        err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
            return les.New(ctx, cfg)//轻节点
        })
    } else {
        err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
            fullNode, err := eth.New(ctx, cfg)// 全节点
...
type Ethereum struct {
    ...
    engine         consensus.Engine				//一致性引擎（共识引擎）。
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
    ...
    eth := &Ethereum{
        ...
        engine:         CreateConsensusEngine(ctx, config, chainConfig, chainDb), // 共识引擎，ethash
...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ethdb.Database) (*ProtocolManager, error) {
    ...
    if mode == downloader.FastSync && blockchain.CurrentBlock().NumberU64() > 0 {
        ...
        mode = downloader.FullSync // 如果本地有block 数据的话，不能使用fastsync
    }
        ...
        manager.SubProtocols = append(manager.SubProtocols, p2p.Protocol{
            ...
            Run: func(p *p2p.Peer, rw p2p.MsgReadWriter) error {
                ...
                case manager.newPeerCh <- peer:
                    ...
                    return manager.handle(peer) //新peer来后的处理
                case <-manager.quitSync:
                    return p2p.DiscQuitting
                }
            },
            NodeInfo: func() interface{} {
                return manager.NodeInfo()  //获取本节点信息
            },
            PeerInfo: func(id discover.NodeID) interface{} {
                if p := manager.peers.Peer(fmt.Sprintf("%x", id[:8])); p != nil {
                    return p.Info() //获取Peer 信息
                }
                return nil
            },
        })
...
type ProtocolManager struct {
    ...
    fetcher    *fetcher.Fetcher // 获取器
    peers      *peerSet // 所有建立连接的peer

    SubProtocols []p2p.Protocol

    eventMux      *event.TypeMux
    txCh          chan core.TxPreEvent // 新交易来了
    txSub         event.Subscription // 新交易的订阅频道
    minedBlockSub *event.TypeMuxSubscription //新生成块儿的订阅频道

    // channels for fetcher, syncer, txsyncLoop
    newPeerCh   chan *peer   //新Peer 来了
    txsyncCh    chan *txsync // 同步交易信号
...
当p2p的server启动的时候，会主动地找节点去连接，或者被其他的节点连接。 连接的过程是`首先进行加密信道的握手`，然后`进行协议的握手`。 `最后为每个协议启动goroutine 执行Run方法来把控制交给最终的协议。` 这个run方法首先创建了一个peer对象，然后调用了handle方法来处理这个peer
...
1. 如果是自己挖的矿。通过g`oroutine minedBroadcastLoop()`来进行广播。
