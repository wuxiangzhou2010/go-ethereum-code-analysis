...
1. 提交但是还不能执行的，放在`queue`里面等待能够执行(比如说nonce太高)。// 同一个地址的交易按nonce排序执行
...
// TxPool 包含了所有当前已知的的交易， 当前网络接收到交易或者本地提交的交易会加入到TxPool。
// 当他们已经被添加到区块链的时候被移除。
...
reset方法检索区块链的当前状态并且确保交易池的内容相对当前的区块链状态是有效的。主要功能包括：
